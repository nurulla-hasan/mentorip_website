...
export default function ServicesPage() {
  return (
    <div className="pb-16 space-y-16">
      ...
      <section className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
           <div className="space-y-2">
              <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary">OUR CAPABILITIES</h2>
              <h3 className="text-3xl md:text-4xl font-bold text-foreground">World-Class Service Suite</h3>
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
           {serviceCategories.map((cat, i) => (
             <Card key={i} className="group border border-border bg-card rounded-2xl p-5 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 relative overflow-hidden flex flex-col justify-between cursor-pointer">
                ...
                <div className="relative space-y-4">
                   <div className="flex items-center justify-between">
                      <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center group-hover:bg-primary transition-all duration-700">
                        <cat.icon className="w-6 h-6 text-muted-foreground group-hover:text-primary-foreground" />
                      </div>
                      <LayoutGrid className="w-4 h-4 text-border" />
                   </div>

                   <div className="space-y-2">
                      <h4 className="text-lg font-bold text-foreground group-hover:text-primary transition-colors">{cat.title}</h4>
                ...
                <div className="relative pt-4 mt-4 border-t border-border flex items-center justify-between group/link">
                   <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">View More</span>
      ...
      {/* Process Section */}
      <section className="bg-muted rounded-3xl p-8 lg:p-12 border border-border shadow-xl relative overflow-hidden group/process">
         <div className="absolute top-0 left-0 w-full h-full opacity-[0.03] dark:opacity-[0.07] pointer-events-none" style={{ backgroundImage: 'radial-gradient(currentColor 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
          ...
          <div className="relative max-w-5xl mx-auto space-y-10">
            <div className="text-center space-y-2">
              <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-primary">HOW WE WORK</h2>
              <h3 className="text-3xl md:text-4xl font-bold text-foreground">Our Methodical Approach</h3>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
               {processes.map((p, i) => (
                 <div key={i} className="relative group/step text-center space-y-6">
                     <div className="w-16 h-16 bg-card rounded-2xl border border-border flex items-center justify-center mx-auto shadow-lg group-hover/step:border-primary transition-all duration-500 relative z-10">
                        <p.icon className="w-6 h-6 text-primary" />
                       <div className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-primary flex items-center justify-center text-[10px] font-bold text-primary-foreground shadow-lg">
                          {p.step}
                       </div>
                    </div>
                     {i < 3 && <div className="hidden md:block absolute top-8 left-[60%] w-[90%] h-0.5 bg-linear-to-r from-primary/30 via-primary/10 to-transparent z-0" />}
                    <div className="space-y-2">
                       <h5 className="text-lg font-bold text-foreground uppercase group-hover/step:text-primary transition-colors">{p.title}</h5>
      ...
      <section className="space-y-6">
         <div className="max-w-4xl mx-auto space-y-8">
             <div className="p-8 lg:p-10 bg-card rounded-3xl border border-border shadow-xl space-y-6 group hover:border-primary/50 hover:shadow-primary/5 transition-all duration-500 relative overflow-hidden">
               ...
               <div className="flex flex-col md:flex-row items-center gap-10">
                  ...
                  <div className="space-y-2 text-center md:text-left">
                     <Badge className="bg-primary/10 text-primary border-0 uppercase tracking-widest text-xs font-bold px-4 py-1.5">Strategic Edge</Badge>
                     <h4 className="text-3xl font-bold text-foreground">Global IP Liaison Office</h4>
             ...
             <div className="p-8 lg:p-10 bg-muted rounded-3xl border border-border shadow-xl space-y-6 group relative overflow-hidden hover:border-primary/50 hover:shadow-primary/5 transition-all duration-500">
               ...
               <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
                  ...
                  <div className="space-y-2 text-center md:text-left">
                     <Badge variant="outline" className="border-primary/40 text-primary uppercase tracking-widest text-xs font-bold px-4 py-1.5">Privacy & Precision</Badge>
                     <h4 className="text-3xl font-bold text-foreground">Full-Spectrum Confidentiality</h4>
                     <p className="text-muted-foreground font-medium text-lg leading-relaxed">
      ...
      <section className="max-w-4xl mx-auto">
         <div className="bg-muted p-6 md:p-10 rounded-3xl border border-border text-center space-y-6 relative overflow-hidden shadow-xl group/cta">
            ...
            <h4 className="text-2xl md:text-3xl font-bold text-foreground uppercase">Ready to Secure Your Innovation?</h4>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-2">
                <Link href="/contact">
                  <Button size="lg" className="rounded-full px-8 py-5 font-bold text-xs uppercase tracking-[0.2em] shadow-lg active:scale-95 group/btn-cta overflow-hidden relative">
                ...
                <Link href="/about">
                  <Button variant="outline" size="lg" className="rounded-full px-8 py-5 font-bold text-xs uppercase tracking-[0.2em] border-border bg-transparent text-foreground hover:bg-muted active:scale-95 transition-all">
                      View Portfolio
                  </Button>
                </Link>
            </div>
